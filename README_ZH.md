...
- **添加新语言的方法**:
  1. 复制 `lang/en.json`（或 `lang/ja.json`），并重命名为目标语言代码（例如韩语为 `ko.json`、法语为 `fr.json`、西班牙语为 `es.json`）。
  2. 将 JSON 顶部的 `"language_name"` 修改为您希望在下拉菜单中显示的名称（例如 `"한국어"`），并翻译各字段文本。
  3. 重新启动应用程序，语言下拉菜单中便会自动加载并显示新语言（完全无需修改任何源代码或重新编译！）。
  *(※ 即使存在尚未翻译的缺失键，程序也不会崩溃，而是会安全地显示为 `[MISSING: 键名]`)*
