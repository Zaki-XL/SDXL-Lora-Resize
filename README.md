...
### 8. LoRA健全性診断 & 色破綻 (CLIP過学習) 自動防止
- ドラッグ＆ドロップ時にファイルの内部構造を高速診断し、WebUI等で生成した際に**「全体が極彩色・白飛び・ネガポジ反転・黒潰れ」**を起こす色破綻リスクや計算異常値を自動検知。
- **検知内容**:
  - Text Encoder (CLIP-L / CLIP-G) の超高学習率（`2e-4` 等）による過学習・重み異常変化
  - `NoobAI-XL Epsilon` や `V-Prediction` 等の特殊ベースモデルによるアーキテクチャ不整合
  - テンソル内の NaN（非数）/ Inf（無限大）破損
- **ユーザー対策選択ダイアログ (`LoRAHealthDialog`)**:
  - **【推奨】Text Encoder 除去 (`_clean_unet`)**: 色破綻の原因となる CLIP 重みを完全除去し、造形・ポーズ（UNet）のみを正常適用（ファイルサイズも約 25%〜35% 軽量化）。
  - **Text Encoder 強度減衰 (`_te_scaled`)**: TE 重みを 0.2 倍にスケール低減して色調崩壊を抑制。
  - **そのまま保持**: WebUI 側で `<lora:名前:1:0>` などの手動調整を行う場合。
  - ※元ファイルは読み取り専用で検査されるため、**元のLoRAファイルが編集・上書きされることは一切ありません**。
